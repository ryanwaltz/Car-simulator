...
def run():
    ...
    print(f"{iterator} cars have passed in {time.time() - now} seconds")


def calculate_time(car):
    timekeeping.append(time.time() - car.start_time)


def calculate_time_end():
    print(sum(timekeeping)/len(timekeeping))
...
def spawn_new():
    ...
    for car in config.cars:
        if math.sqrt((car.pos[0] - position[0])**2 + (car.pos[1] - position[1])**2) < 60:
            ...
            print("prohibited spawn")
            break
    if continue_create:
        config.cars.append(
            Car((255, 0, 0), position, angle, 50, 20, 1, road=config.roads[road], lane=lane, intent=[config.roads[intent_road], random.choice([1, 0, -1])])
        )
    else:
        spawn_new()
...
config.cars = [
    Car((255, 0, 0), [30, 30], angle=0, length=50, width=20, velocity=1, road=config.roads[1], lane=3, intent=[config.roads[0], -1]),
    Car((255, 0, 0), [0, 0], angle=0, length=50, width=20, velocity=1, road=config.roads[1], lane=3, intent=[config.roads[0], 0]),
        ]
